...
then
   ...
   echo "Generating CSR ..."
   #sudo /opt/eris/bin/eris-generate-csr-v2.exp "US" "MA" "Boston" "Partners Healthcare" "ERIS" "$CERTIFICATEDN" "erisalerts@partners.org" "$CHALLENGE" "PHS" "$ERISPATH/$CERTIFICATEDN/$CERTIFICATEDN.key" "$ERISPATH/$CERTIFICATEDN/$CERTIFICATEDN.csr" 
   sudo /opt/eris/bin/eris-generate-csr-v2.exp "US" "MA" "Boston" "Partners Healthcare" "ERIS" "*.$CERTIFICATEDN" "erisalerts@partners.org" "$CHALLENGE" "PHS" "$ERISPATH/$CERTIFICATEDN/$CERTIFICATEDN.key" "$ERISPATH/$CERTIFICATEDN/$CERTIFICATEDN.csr"
   echo "CSR generated."
   echo "-------------------------------------------------------------------------------------------"
